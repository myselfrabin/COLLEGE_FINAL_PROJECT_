...
const cityMarker = { lat: 27.1715, lng: 87.0574, icon: icons.city, name: "Bhojpur Bazaar", desc: "District town center", img: "" };

const mostFamous: Place[] = [
  { lat: 27.1715, lng: 87.0574, icon: icons.famous, name: "Bhojpur Bazaar", desc: "Main market area.", img: "" },
  { lat: 27.2000, lng: 87.0500, icon: icons.famous, name: "View ridge", desc: "Scenic lookout.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 27.1500, lng: 87.0300, icon: icons.hidden, name: "Village trail", desc: "Quiet local path.", img: "" },
];

const nearSpots: Place[] = [
  { lat: 27.2200, lng: 87.0900, icon: icons.near, name: "Nearby hamlet", desc: "Small settlement.", img: "" },
];
...
export default function BhojpurMap() {
  ...
  return (
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      <MapContainer center={[cityMarker.lat, cityMarker.lng]} zoom={12} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }} whenCreated={(mapInstance) => (mapRef.current = mapInstance)} className="z-0">
        ...
        {routeLine.length > 0 && (<Polyline positions={routeLine} pathOptions={{ color: "#ef4444", weight: 5, opacity: 0.9 }} />)}
        <MapClickHandler />
      </MapContainer>
      ...
      {showPlanner ? (
        <div className="absolute top-5 left-5 bg-white/95 backdrop-blur rounded-lg shadow-lg p-3 z-[999] w-[260px] max-h-[70%] overflow-auto">
          ...
          <div className="flex gap-2"><button className="bg-red-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={selectedStops.length === 0} onClick={buildAndDrawRoute}>Optimize & Route</button><button className="bg-gray-200 px-3 py-1 rounded" onClick={clearRoute}>Clear</button></div>
