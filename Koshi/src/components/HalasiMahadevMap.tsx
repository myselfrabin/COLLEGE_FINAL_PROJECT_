...
const cityMarker = { lat: 27.1903, lng: 86.6317, icon: icons.city, name: "Halesi Mahadev", desc: "Sacred cave temple", img: "" };

const mostFamous: Place[] = [
  { lat: 27.1903, lng: 86.6317, icon: icons.famous, name: "Halesi Cave", desc: "Main shrine and cave complex.", img: "" },
  { lat: 27.2000, lng: 86.6400, icon: icons.famous, name: "Viewpoint ridge", desc: "Hill with views.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 27.1850, lng: 86.6200, icon: icons.hidden, name: "Forest path", desc: "Quiet walking trail.", img: "" },
];

const nearSpots: Place[] = [
  { lat: 27.2400, lng: 86.6700, icon: icons.near, name: "Nearby village", desc: "Local settlement.", img: "" },
];

const Legend: React.FC = () => (
  <div className="absolute bottom-5 right-5 bg-white rounded-lg shadow-lg p-3 z-[999] text-sm w-[120px]">
    <div className="flex items-center mb-1"><img src={icons.city.options.iconUrl} className="w-4 mr-2" /> Center</div>
...
export default function HalasiMahadevMap() {
    ...
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      <MapContainer center={[cityMarker.lat, cityMarker.lng]} zoom={12} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }} whenCreated={(mapInstance) => (mapRef.current = mapInstance)} className="z-0">
        ...
        <LayersControl position="topright">
          ...
          <LayersControl.Overlay name="Nearby Spots">
            <LayerGroup>
              {nearSpots.map((p, i) => (
                <Marker key={i} position={[p.lat, p.lng]} icon={p.icon} eventHandlers={{ click: () => setSelectedPlace(p) }}>
                  <Popup><b>{p.name}</b><br />{p.desc}</Popup>
        ...
        {routeLine.length > 0 && (<Polyline positions={routeLine} pathOptions={{ color: "#14b8a6", weight: 5, opacity: 0.9 }} />)}
        <MapClickHandler />
      </MapContainer>
      ...
      {showPlanner ? (
        <div className="absolute top-5 left-5 bg-white/95 backdrop-blur rounded-lg shadow-lg p-3 z-[999] w-[260px] max-h-[70%] overflow-auto">
          <div className="flex items-center justify-between mb-2"><div className="font-semibold">Plan your route</div><button className="text-xs px-2 py-1 bg-gray-200 rounded" onClick={() => setShowPlanner(false)}>Hide</button></div>
          <div className="text-xs text-gray-600 mb-2">Start: {cityMarker.name}</div>
          <div className="space-y-1 mb-3">{allPlaces.map((p) => (<label key={p.name} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!selectedStops.find((s) => s.name === p.name)} onChange={() => toggleStop(p)} /><span>{p.name}</span></label>))}</div>
          <div className="flex gap-2"><button className="bg-teal-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={selectedStops.length === 0} onClick={buildAndDrawRoute}>Optimize & Route</button><button className="bg-gray-200 px-3 py-1 rounded" onClick={clearRoute}>Clear</button></div>
        ...
        <button className="absolute top-5 left-5 bg-white rounded shadow px-3 py-1 z-[999] text-sm" onClick={() => setShowPlanner(true)}>Show planner</button>
      )}
      <Legend />
