...
const cityMarker = { lat: 27.7025, lng: 88.1475, icon: icons.city, name: "Kanchenjunga Region", desc: "Trekking area hub", img: "" };

const mostFamous: Place[] = [
  { lat: 27.6015, lng: 88.0887, icon: icons.famous, name: "Taplejung (Phungling)", desc: "Gateway town to treks.", img: "" },
  { lat: 27.6956, lng: 88.1273, icon: icons.famous, name: "Ghunsa", desc: "High-altitude village on trail.", img: "" },
  { lat: 27.7200, lng: 88.1475, icon: icons.famous, name: "Pangpema (North BC)", desc: "Viewpoint of Kanchenjunga.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 27.6680, lng: 88.1160, icon: icons.hidden, name: "Lhonak Valley", desc: "Wild high valley.", img: "" },
];

const nearSpots: Place[] = [
  { lat: 27.5250, lng: 88.0760, icon: icons.near, name: "Pathibhara Temple", desc: "Famous pilgrimage site.", img: "" },
];

const Legend: React.FC = () => (
  <div className="absolute bottom-5 right-5 bg-white rounded-lg shadow-lg p-3 z-[999] text-sm w-[120px]">
    <div className="flex items-center mb-1"><img src={icons.city.options.iconUrl} className="w-4 mr-2" /> Region</div>
...
export default function KanchenjungaMap() {
  ...
  function toggleStop(place: Place) {
    setRouteLine([]); setRouteStats(null);
  ...
  return (
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      <MapContainer center={[cityMarker.lat, cityMarker.lng]} zoom={10} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }} whenCreated={(mapInstance) => (mapRef.current = mapInstance)} className="z-0">
        ...
        <LayersControl position="topright">
          ...
          <LayersControl.Overlay name="Locations Near Kanchenjunga">
            <LayerGroup>
              {nearSpots.map((p, i) => (
                <Marker key={i} position={[p.lat, p.lng]} icon={p.icon} eventHandlers={{ click: () => setSelectedPlace(p) }}>
                  <Popup><b>{p.name}</b><br />{p.desc}</Popup>
        ...
        {routeLine.length > 0 && (<Polyline positions={routeLine} pathOptions={{ color: "#f97316", weight: 5, opacity: 0.9 }} />)}
        <MapClickHandler />
      </MapContainer>
      ...
      {showPlanner ? (
        <div className="absolute top-5 left-5 bg-white/95 backdrop-blur rounded-lg shadow-lg p-3 z-[999] w-[260px] max-h-[70%] overflow-auto">
          ...
          <div className="space-y-1 mb-3">
            {[...mostFamous, ...hiddenDestinations, ...nearSpots].map((p) => (
              <label key={p.name} className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={!!selectedStops.find((s) => s.name === p.name)} onChange={() => toggleStop(p)} />
                <span>{p.name}</span>
              </label>
            ))}
          </div>
          <div className="flex gap-2">
            <button className="bg-orange-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={selectedStops.length === 0} onClick={buildAndDrawRoute}>Optimize & Route</button>
