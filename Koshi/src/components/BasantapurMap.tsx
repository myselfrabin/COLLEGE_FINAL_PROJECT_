...
// Basantapur (Terhathum) area center
const cityMarker = { lat: 27.0385, lng: 87.6765, icon: icons.city, name: "Basantapur (Terhathum)", desc: "Hill station and trails", img: "" };

const mostFamous: Place[] = [
  { lat: 27.0385, lng: 87.6765, icon: icons.famous, name: "Basantapur Bazaar", desc: "Town center.", img: "" },
  { lat: 27.0650, lng: 87.6590, icon: icons.famous, name: "Tinjure Milke Jaljale", desc: "Rhododendron area & ridge.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 27.0200, lng: 87.7000, icon: icons.hidden, name: "Quiet forest trail", desc: "Less-traveled path.", img: "" },
];

const nearSpots: Place[] = [
  { lat: 27.1350, lng: 87.6050, icon: icons.near, name: "Hile (Sankhuwasabha)", desc: "Nearby hill town.", img: "" },
];
...
export default function BasantapurMap() {
    ...
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      <MapContainer center={[cityMarker.lat, cityMarker.lng]} zoom={12} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }} whenCreated={(mapInstance) => (mapRef.current = mapInstance)} className="z-0">
        ...
        <LayersControl position="topright">
          ...
          <LayersControl.Overlay name="Locations Near Basantapur">
            <LayerGroup>
              {nearSpots.map((p, i) => (
                <Marker key={i} position={[p.lat, p.lng]} icon={p.icon} eventHandlers={{ click: () => setSelectedPlace(p) }}>
                  <Popup><b>{p.name}</b><br />{p.desc}</Popup>
        ...
        {routeLine.length > 0 && (<Polyline positions={routeLine} pathOptions={{ color: "#6366f1", weight: 5, opacity: 0.9 }} />)}
        <MapClickHandler />
      </MapContainer>
      {showPlanner ? (
        <div className="absolute top-5 left-5 bg-white/95 backdrop-blur rounded-lg shadow-lg p-3 z-[999] w-[260px] max-h-[70%] overflow-auto">
          ...
          <div className="flex gap-2"><button className="bg-indigo-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={selectedStops.length === 0} onClick={buildAndDrawRoute}>Optimize & Route</button><button className="bg-gray-200 px-3 py-1 rounded" onClick={clearRoute}>Clear</button></div>
