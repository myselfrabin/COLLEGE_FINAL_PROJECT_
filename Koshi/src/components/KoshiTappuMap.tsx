...
const cityMarker = {
  ...
  desc: "Wetlands and birdlife.",
  img: "",
};

const mostFamous: Place[] = [
  { lat: 26.647, lng: 87.053, icon: icons.famous, name: "Reserve HQ Area", desc: "Entry and info point.", img: "" },
  { lat: 26.632, lng: 87.095, icon: icons.famous, name: "Koshi Barrage", desc: "Sprawling barrage on Koshi river.", img: "" },
  { lat: 26.665, lng: 87.030, icon: icons.famous, name: "Bird Watching Point", desc: "Popular spot for migratory birds.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 26.622, lng: 87.020, icon: icons.hidden, name: "Wetland Backwater", desc: "Quiet backchannel habitat.", img: "" },
];

const nearSpots: Place[] = [
  { lat: 26.658, lng: 87.154, icon: icons.near, name: "Chatara", desc: "Ghat and market area.", img: "" },
];
...
export default function KoshiTappuMap() {
    ...
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      ...
      >
        ...
        <LayersControl position="topright">
          ...
          <LayersControl.Overlay name="Locations Near Koshi Tappu">
            <LayerGroup>
              {nearSpots.map((p, i) => (
                <Marker key={i} position={[p.lat, p.lng]} icon={p.icon} eventHandlers={{ click: () => setSelectedPlace(p) }}>
                  <Popup>
        ...
        {routeLine.length > 0 && (
          <Polyline positions={routeLine} pathOptions={{ color: "#0ea5e9", weight: 5, opacity: 0.9 }} />
        )}
        <MapClickHandler />
      </MapContainer>
      ...
      {showPlanner ? (
        <div className="absolute top-5 left-5 bg-white/95 backdrop-blur rounded-lg shadow-lg p-3 z-[999] w-[260px] max-h-[70%] overflow-auto">
          ...
          <div className="flex gap-2">
            <button className="bg-blue-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={selectedStops.length === 0} onClick={buildAndDrawRoute}>Optimize & Route</button>
