...
// Center of Ilam Bazaar
const cityMarker = {
  lat: 26.91118,
  lng: 87.92367,
  icon: icons.city,
  name: "Ilam Bazaar",
  desc: "Welcome to Ilam!",
  img: "",
};

// Landmarks in Ilam and nearby
const mostFamous: Place[] = [
  { lat: 26.91118, lng: 87.92367, icon: icons.famous, name: "Ilam Bazaar", desc: "Main town center.", img: "" },
  { lat: 26.9117, lng: 87.9605, icon: icons.famous, name: "Kanyam Tea Garden", desc: "Iconic rolling tea estates.", img: "" },
  { lat: 26.9560, lng: 88.0435, icon: icons.famous, name: "Antu Danda (Shree Antu)", desc: "Sunrise viewpoint over tea gardens.", img: "" },
  { lat: 26.8854, lng: 87.9552, icon: icons.famous, name: "Fikkal", desc: "Bustling junction town.", img: "" },
  { lat: 27.0387, lng: 88.1122, icon: icons.famous, name: "Maipokhari", desc: "Sacred lake surrounded by forest.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 26.9325, lng: 87.9054, icon: icons.hidden, name: "Siddhi Thumka", desc: "Quiet viewpoint ridge.", img: "" },
  { lat: 26.9010, lng: 87.8800, icon: icons.hidden, name: "Hile Bazar Road View", desc: "Local scenic drive area.", img: "" },
];

const nearIlam: Place[] = [
  { lat: 26.9672, lng: 87.6645, icon: icons.near, name: "Pathibhara (Taplejung side)", desc: "Pilgrimage destination (far).", img: "" },
  { lat: 27.1726, lng: 87.9310, icon: icons.near, name: "Sandakpur", desc: "Ridge with views of Kanchenjunga.", img: "" },
];

const Legend: React.FC = () => (
  <div className="absolute bottom-5 right-5 bg-white rounded-lg shadow-lg p-3 z-[999] text-sm w-[120px]">
    <div className="flex items-center mb-1"><img src={icons.city.options.iconUrl} className="w-4 mr-2" /> Town Center</div>
...
export default function IllamMap() {
    ...
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      ...
      >
        ...
        <LayersControl position="topright">
          ...
          <LayersControl.Overlay name="Locations Near Ilam">
            <LayerGroup>
              {nearIlam.map((p, i) => (
                <Marker
                  key={i}
        ...
        {routeLine.length > 0 && (
          <Polyline positions={routeLine} pathOptions={{ color: "#16a34a", weight: 5, opacity: 0.9 }} />
        )}
        <MapClickHandler />
      </MapContainer>
