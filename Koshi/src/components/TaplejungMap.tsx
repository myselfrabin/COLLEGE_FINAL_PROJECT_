...
const cityMarker = { lat: 27.3546, lng: 87.6762, icon: icons.city, name: "Taplejung (Phungling)", desc: "Gateway to Kanchenjunga treks", img: "" };

const mostFamous: Place[] = [
  { lat: 27.3546, lng: 87.6762, icon: icons.famous, name: "Phungling Bazaar", desc: "Town center.", img: "" },
  { lat: 27.5110, lng: 87.7350, icon: icons.famous, name: "Pathibhara Temple", desc: "Pilgrimage site.", img: "" },
];

const hiddenDestinations: Place[] = [
  { lat: 27.4000, lng: 87.7000, icon: icons.hidden, name: "Village route", desc: "Quiet route toward hills.", img: "" },
];

const nearSpots: Place[] = [
  { lat: 27.3000, lng: 87.6000, icon: icons.near, name: "Nearby hamlet", desc: "Local settlement.", img: "" },
];
...
export default function TaplejungMap() {
  ...
  return (
    <div className="relative mt-6 h-[300px] w-full max-w-[575px] rounded-lg overflow-hidden mx-auto">
      <MapContainer center={[cityMarker.lat, cityMarker.lng]} zoom={12} scrollWheelZoom={true} style={{ height: "100%", width: "100%" }} whenCreated={(mapInstance) => (mapRef.current = mapInstance)} className="z-0">
        ...
        {routeLine.length > 0 && (<Polyline positions={routeLine} pathOptions={{ color: "#10b981", weight: 5, opacity: 0.9 }} />)}
        <MapClickHandler />
      </MapContainer>
      ...
      {showPlanner ? (
        <div className="absolute top-5 left-5 bg-white/95 backdrop-blur rounded-lg shadow-lg p-3 z-[999] w-[260px] max-h-[70%] overflow-auto">
          ...
          <div className="flex gap-2"><button className="bg-emerald-600 text-white px-3 py-1 rounded disabled:opacity-50" disabled={selectedStops.length === 0} onClick={buildAndDrawRoute}>Optimize & Route</button><button className="bg-gray-200 px-3 py-1 rounded" onClick={clearRoute}>Clear</button></div>
